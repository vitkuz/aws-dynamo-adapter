...
async function testFetchOneRecord() {
  ...
  try {
    ...
    const fetched = await adapter.fetchOneRecord<{
      id: string;
      sk: string;
      name: string;
      price: number;
    }>({ id: testId, sk: 'products' });
    ...
    if (fetched.name !== 'Test Product for Fetch')
      throw new Error(`Name mismatch: ${fetched.name} !== Test Product for Fetch`);
    ...
    const notFound = await adapter.fetchOneRecord<{
      id: string;
      sk: string;
      name: string;
      price: number;
    }>({ id: nonExistentId, sk: 'products' });
    ...
    const deletedRecord = await adapter.fetchOneRecord<{
      id: string;
      sk: string;
      name: string;
      price: number;
