...
runner.test('should use custom logger implementation', async () => {
  ...
  const adapterCreatedLog = logMessages.find(
    (log) => log.level === 'info' && log.message === 'DynamoDB adapter created'
  );
  ...
  const createLog = logMessages.find(
    (log) => log.level === 'info' && log.message === 'Record created successfully'
  );
  ...
  const deleteLog = logMessages.find(
    (log) => log.level === 'info' && log.message === 'Record deleted successfully'
  );
...
runner.test('should use both custom client and custom logger', async () => {
  ...
  assert.includes(
    logMessages[0],
    '[INFO] DynamoDB adapter created',
    'Should use custom logger format'
  );
