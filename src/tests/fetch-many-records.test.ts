  ...
  try {
    ...
    for (const author of createdAuthors) {
      ...
      if (found.name !== author.name)
        throw new Error(`Author name mismatch: ${found.name} !== ${author.name}`);
      if (found.bio !== author.bio) throw new Error(`Author bio mismatch`);
      if (!found.createdAt || !found.updatedAt)
        throw new Error(`Author ${author.id} missing timestamps`);
    }
    console.log('✅ All authors fetched successfully');
    console.log(
      'Fetched authors:',
      fetchedAuthors.map((a) => ({ id: a.id, name: a.name }))
    );
    ...
    for (const book of createdBooks) {
      ...
      if (found.title !== book.title)
        throw new Error(`Book title mismatch: ${found.title} !== ${book.title}`);
      ...
      if (!found.createdAt || !found.updatedAt)
        throw new Error(`Book ${book.id} missing timestamps`);
    }
    console.log('✅ All books fetched successfully');
    console.log(
      'Fetched books:',
      fetchedBooks.map((b) => ({ id: b.id, title: b.title }))
    );
