  ...
  try {
    ...
    for (let i = 0; i < createdAuthors.length; i++) {
      ...
      if (created.id !== original.id)
        throw new Error(`Author ${i} ID mismatch: ${created.id} !== ${original.id}`);
      if (created.sk !== 'authors')
        throw new Error(`Author ${i} SK mismatch: ${created.sk} !== authors`);
      if (created.name !== original.name)
        throw new Error(`Author ${i} name mismatch: ${created.name} !== ${original.name}`);
      ...
      if (created.createdAt !== created.updatedAt)
        throw new Error(`Author ${i} timestamps should be equal on creation`);
    }

    console.log('✅ Authors batch created successfully');
    console.log(
      'Created authors:',
      createdAuthors.map((a) => ({ id: a.id, name: a.name }))
    );
    ...
    for (let i = 0; i < createdBooks.length; i++) {
      ...
      if (created.id !== original.id)
        throw new Error(`Book ${i} ID mismatch: ${created.id} !== ${original.id}`);
      if (created.sk !== 'books') throw new Error(`Book ${i} SK mismatch: ${created.sk} !== books`);
      if (created.title !== original.title)
        throw new Error(`Book ${i} title mismatch: ${created.title} !== ${original.title}`);
      if (created.authorId !== original.authorId)
        throw new Error(
          `Book ${i} authorId mismatch: ${created.authorId} !== ${original.authorId}`
        );
      if (created.isbn !== original.isbn)
        throw new Error(`Book ${i} ISBN mismatch: ${created.isbn} !== ${original.isbn}`);
      ...
      if (created.createdAt !== created.updatedAt)
        throw new Error(`Book ${i} timestamps should be equal on creation`);
    }

    console.log('✅ Books batch created successfully');
    console.log(
      'Created books:',
      createdBooks.map((b) => ({ id: b.id, title: b.title, authorId: b.authorId }))
    );
    ...
    for (let i = 0; i < books.length; i++) {
      ...
      if (book.authorId !== expectedAuthorId) {
        throw new Error(
          `Book "${book.title}" has incorrect authorId: ${book.authorId} !== ${expectedAuthorId}`
        );
      }
    }
    ...
    if (foundAuthors.length !== 3)
      throw new Error(`Expected to find 3 authors, found ${foundAuthors.length}`);
    ...
    if (foundBooks.length !== 3)
      throw new Error(`Expected to find 3 books, found ${foundBooks.length}`);
